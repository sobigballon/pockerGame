# allmax(iterable, key=None), that returns
    ...
    while i<len(iterable)-1:
        while j<len(iterable):
            if key(iterable[i])==key(iterable[j]):
                if iterable[i] not in ties:
                    ties.append(iterable[i])
                if iterable[j] not in ties:
                    ties.append(iterable[j])
            j+=1
        i+=1
#    print ties
    for e in iterable:
    ...
    for entry in iterable:
        if key(entry)==max(output):
            only_winnner=entry
    return ties if (ties) else only_winnner
